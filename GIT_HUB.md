...
-Podemos também checar uma documentação do projeto pelo README.md;
-Os detalhes da licença do projeto;
-Criar branches, adicionar arquivos;

Issue
-Na aba Issue podemos criar tarefas ou póssíveos bugs do projeto;
...
-Podemos utilizar o Markdown no texto também (iguao o README.md);
-A issue deve ter uma label e também um responsavel;

Pull request
-Na aba Pull Request é onde os colaboradores do projeto enviam código para resolver as issues ou adicionar
...
-O pull request vem de um novo branch, criado no projeto e enviado para o repo, com o incremeto de codigo;

Actions
-É onde se cria as automatizações de deploy com integração em outros serviços;
-Incluindo CI/CD (Continuos Integration / Continuos Development);
...
-A ideia é que seja um repositorio de conhecimento sobre o projeto e centralizar as informações em um lugar so;

Insights
-Na aba Insights temos informações detalhadas do projeto, como:
-Quem são os contribuidores, commits, forks e muito mais;
-Interessante para entender como o projeto esta andando e a sua evolução desde o inicio;
...
-O repositorio pode ser removido nesta aba;

Gist
-Gists são pequenos blocos de codigo que podem ser hospedado no GitHub tambem;
-Você pode armazenar uma solução que achou interessante para algum problema e não quer perder, por exemplo;
-No fim das contas o Gist acaba sendo um repositorio tambem que vai ter uma solução para um problema bem especifico;
...
1 - Criar um repositório com o nome - ex.: nomedousuario.github.io;
2 - Clonas o repositorio no nosso computador;
3 - Adicionar o código do projeto na branch master;
4 - Enviar o códgio do projeto por meio de push;
